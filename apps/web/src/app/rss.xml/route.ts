...
export function GET() {
  const items = [...guides, ...articles].sort((a, b) => b.publishedAt.localeCompare(a.publishedAt)).map((item) => {
    ...
    return `<item><title>${xml(item.title)}</title><link>${xml(absoluteUrl(`${base}/${item.slug}`))}</link><guid>${xml(absoluteUrl(`${base}/${item.slug}`))}</guid><description>${xml(item.description)}</description><pubDate>${new Date(editorialDateIso(item.publishedAt)).toUTCString()}</pubDate><author>${xml(item.author)}</author></item>`;
  }).join("");
