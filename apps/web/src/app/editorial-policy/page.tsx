...
].map(([index, title, copy]) => <section className="surface-card" key={index}><span>{index}</span><h2>{title}</h2><p>{copy}</p></section>)}</div><section className="surface-card policy-note"><h2>استفاده از منابع دیگر</h2><p>مقاله، پاسخ تشریحی، کتاب یا محتوای اختصاصی رقبا کپی نمی‌شود. منابع بیرونی برای اعتبارسنجی واقعیت، مقایسه و استناد استفاده می‌شوند. محتوای دو کانال رسمی متعلق به kunkur01 است و هنگام تبدیل به مقاله، لینک پست اصلی حفظ می‌شود.</p></section></main>; }
